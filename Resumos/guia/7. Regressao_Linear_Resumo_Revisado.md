...
Ridge utiliza penalização L2 e reduz a magnitude dos coeficientes, mas normalmente não os zera. É interessante quando temos multicolinearidade ou vários preditores com pequenos efeitos e queremos mantê-los no modelo. Já o Lasso utiliza penalização L1 e pode levar alguns coeficientes exatamente a zero, realizando regularização e seleção de features simultaneamente. É útil principalmente em problemas de alta dimensionalidade nos quais esperamos que apenas parte das variáveis seja realmente relevante.
